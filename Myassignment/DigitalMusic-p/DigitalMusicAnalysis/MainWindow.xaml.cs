...
namespace DigitalMusicAnalysis
{
    public partial class MainWindow : Window
    {
        ...
        public MainWindow()
        {
            ...
            //starting timer for loadhistogram function
            var timerloadhistogram = new Stopwatch();
            timerloadhistogram.Start();
            Console.Out.Write("timer started for loadhistogram  \n");
            loadHistogram();
            timerloadhistogram.Stop();
            Console.Out.Write("lopadhistogram  timer ended. Time elapsed: {0} \n", timerloadhistogram.ElapsedMilliseconds);



            //starting timer for playback function
            var timerplayback = new Stopwatch();
            timerplayback.Start();
            Console.Out.Write("timer started for playback  \n");
            playBack();
            timerplayback.Stop();
            Console.Out.Write("playback  timer ended. Time elapsed: {0} \n", timerplayback.ElapsedMilliseconds);


            //starting timer for playback function
            var timercheckstart = new Stopwatch();
            timercheckstart.Start();
            Console.Out.Write("timer started for checkstart  \n");
            check.Start();
            timercheckstart.Stop();
            Console.Out.Write("checkstart  timer ended. Time elapsed: {0} \n", timercheckstart.ElapsedMilliseconds);


            //stopping timer for whole program
            timerwholeprogram.Stop();
            Console.Out.Write("whole program timer ended. Time elapsed: {0} \n", timerwholeprogram.ElapsedMilliseconds);
        ...
        private void onsetDetection()
        {
            ...
            for (int mm = 0; mm < lengths.Count; mm++)
            {
                ...
                Parallel.For (0, nearest, new ParallelOptions { MaxDegreeOfParallelism = numberofprocessor }, ll => {
        ...
        //
        //NO NEEDS OF THIS FFT BECAUSE NEW FFT IS CALLED FROM FFT.CS AND THANK GO IT'S WORKING SO :)
        //
        ...
        private musicNote[] readXML(string filename)
        {
            ...
            {
                //System.Console.Write("file found");
            }
        ...
        //not working because of pitches. it's trying to access data at th same time. 
        //public void fftOnSetdetection(object data)
        //{
        //    //Complex[] Y;
        //    //Complex[] twiddles;
        //    //double[] absY;
        ...
        //    int threadId = (int)data;
        //    int CHUNK_SIZE = lengths.Count/ numberofprocessor;
        //    int start = threadId * CHUNK_SIZE;
        //    int finish = Math.Min(start + CHUNK_SIZE, lengths.Count);
        //    System.Console.Out.Write("fftonsetdet thread {0}, {1} -> {2}\n", threadId, start, finish);

        //    for (int mm = start; mm < finish; mm++)
        //    {
        //        int nearest = (int)Math.Pow(2, Math.Ceiling(Math.Log(lengths[mm], 2)));
        ...
        //        {
        //            double a = 2 * Math.PI * ll / (double)nearest;
        //            twiddles[ll] = Complex.Pow(Complex.Exp(-i), (float)a);
        //        }
